...
const WriteArticle = () => {
  const articleLength = [
    {length: 1200, text: 'Short (500-800 words)'},
  ...
  const onSubmitHandler= async (e) => {
    ...
    try {
      setLoading(true)
      const prompt = `Write an article about ${input}. The article should have an 
      introduction, detailed body, and a well-written conclusion that summarizes the main points.
       The total length should be ${selectedLength.text}. Make sure the article ends with a proper 
       conclusion.`;
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
          ...
          type='text' className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border
          ...
          <br />
          <button disabled = {loading} className='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#226BFF] to-[#65ADFF] text-white
